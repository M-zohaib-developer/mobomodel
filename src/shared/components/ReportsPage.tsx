  ...
  const getCompletedDevices = () => {
    if (currentUser?.role === "admin") {
      ...
      return devices.filter(
        (d) => d.clientId === currentUser?.id && d.status === "completed"
      );
    }
  };
      ...
      const htmlContent = `
        ...
        <body>
          ...
          <div class="section">
            ...
            <p><span class="label">Original Issue:</span> ${
          ...
          <div class="section">
            ...
            <p><span class="label">Completion Date:</span> ${new Date(
              device.updatedAt
...
Completion Date: ${new Date(device.updatedAt).toLocaleDateString()}
  ...
  const downloadBulkReports = () => {
    if (currentUser?.role === "enterprise" || currentUser?.role === "admin") {
        ...
        ...filteredDevices.map(
          (device) =>
            `"${device.brand}","${device.model}","${device.imei}","${
              ...
              device.updatedAt
            ).toLocaleDateString()}","${device.notes.join("; ")}"`
        ),
      ].join("\n");
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `MoboCheck_Bulk_Report_${
