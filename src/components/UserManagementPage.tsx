...
import { Users, Plus, Edit, Trash2, Search, UserCheck, Building, Shield } from 'lucide-react';
...
export function UserManagementPage({ onNavigate }: UserManagementPageProps) {
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      case 'client': return 'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100';
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Clients', count: users.filter(u => u.role === 'client').length, color: 'text-green-600' },
            { label: 'Enterprises', count: users.filter(u => u.role === 'enterprise').length, color: 'text-purple-600' },
            { label: 'Admins', count: users.filter(u => u.role === 'admin').length, color: 'text-red-600' }
          ].map((stat, index) => (
