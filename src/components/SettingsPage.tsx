...
import { Settings, Moon, Sun, Bell, Save, Globe, Shield, Trash2, User } from 'lucide-react';
...
export function SettingsPage({ onNavigate }: SettingsPageProps) {
  ...
  const handleSaveSettings = () => {
    ...
    if (profileData.name !== currentUser?.name || profileData.avatar !== currentUser?.avatar) {
      dispatch({ 
        type: 'UPDATE_USER', 
        payload: {
  ...
  const handleClearData = () => {
    if (window.confirm('This will delete all your data. Are you sure?')) {
      ...
      if (currentUser?.role === 'admin') {
        ...
        const userDevices = state.devices.filter(d => d.clientId !== currentUser?.id);
        const userOrders = state.orders.filter(o => o.clientId !== currentUser?.id);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className={`block text-sm font-medium mb-2 ${
                  localSettings.theme === 'dark' ? 'text-gray-300' : 'text-gray-700'
                }`}>
                  Full Name
                </label>
                <input
              ...
              <div className="md:col-span-2">
                <div className={`p-3 rounded-lg ${
                  localSettings.theme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'
                }`}>
                  <p className={`text-sm ${
                    localSettings.theme === 'dark' ? 'text-gray-300' : 'text-gray-600'
                  }`}>
                    <span className="font-medium">Role:</span> {currentUser?.role?.toUpperCase()}
                  </p>
                  <p className={`text-sm ${
                    localSettings.theme === 'dark' ? 'text-gray-300' : 'text-gray-600'
                  }`}>
                    <span className="font-medium">Member Since:</span> {currentUser ? new Date(currentUser.createdAt).toLocaleDateString() : 'N/A'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-sm ${
                    localSettings.theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
                  }`}>
              ...
              <div className="flex items-center justify-between py-3">
                <div>
                  <h3 className={`font-medium ${
                    ...
                    Clear {currentUser?.role === 'admin' ? 'All System' : 'My'} Data
                  </h3>
                  <p className={`text-sm ${
                    localSettings.theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
                  }`}>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className={`w-full max-w-md rounded-xl p-6 ${
              ...
              <p className={`mb-6 ${
                ...
                This action cannot be undone. This will permanently delete your account and remove all associated data from the system.
              </p>
